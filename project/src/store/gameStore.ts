  ...
  switch (model.type) {
    ...
    case GameType.COMPLETE_DYNAMIC:
      if (step % 2 === 0) {
        ...
        if (prevResults.length > 0) {
          ...
          result.signals = { ...prevResult.signals };
        }
      }
    ...
    case GameType.INCOMPLETE_STATIC:
      ...
      if (step === 0) {
        ...
        const prevResults = store.simulationState.results;
        const prevResult = prevResults[prevResults.length - 1];
        result.beliefs = updateBeliefs(model, prevResult);
      }
    ...
    case GameType.INCOMPLETE_DYNAMIC:
      if (step === 0) {
        ...
        // 继承上一回合的发送者类型
        const prevResults = store.simulationState.results;
        result.signals['senderType'] = prevResults[prevResults.length - 1].signals['senderType'];
      }
...
function calculateDynamicStrategy(model: GameModel, playerId: number, step: number, store: GameStore): number {
  ...
  const expectedPayoffs = model.players[0].strategies.map((strategy, index) => {
...
function calculateFollowerResponse(model: GameModel, leaderChoice: string): number {
  const followerPayoffs = model.players[1].strategies.map((strategy, index) => {
...
function updateBeliefs(model: GameModel, prevResult: SimulationResult): { [key: number]: { [key: string]: number } } {
  const beliefs: { [key: number]: { [key: string]: number } } = {};
  
  model.players.forEach(player => {
    beliefs[player.id] = {};
    const prevBeliefs = prevResult.beliefs[player.id];
    const prevPayoff = prevResult.payoffs[player.id];
    
    let totalWeight = 0;
    player.strategies.forEach(strategy => {
      const weight = prevBeliefs[strategy] * Math.exp(prevPayoff / 10);
...
function calculateIncompleteInfoStrategy(
  ...
  const expectedPayoffs = model.players[playerId - 1].strategies.map((strategy, index) => {
